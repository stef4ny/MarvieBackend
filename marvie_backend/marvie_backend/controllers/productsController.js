...
exports.getProducts = (req, res) => {
  ...
  exports.createProduct = (req, res) =>{
    const {nome, descricao, estoque, data_fabricacao, valor, cat_id, status} = req.body;
    const sql = 'INSERT INTO products (nome, descricao, estoque, data_fabricacao, valor, cat_id, status) VALUES (?,?,?,?,?,?,?)';
    const values = [nome, descricao, estoque, data_fabricacao, valor, cat_id, status];
...
exports.putProducts = (req, res) =>{
  ...
  const {nome, descricao, estoque, data_fabricacao, valor, cat_id, status} = req.body;
  db.query('UPDATE products SET nome = ?, descricao = ?, estoque = ?, data_fabricacao = ?, valor = ?, cat_id = ?, status = ?', [nome, descricao, estoque, data_fabricacao, valor, cat_id,status, id], (err, results) =>{
    if(err){
      res.status(500).send(err.message);
      return;
    }
...
exports.postReviews = (req, res)=>{
  ...
  const sql = 'INSERT INTO reviewws (product_id, user_id, rating) VALUES (?,?,?)';
